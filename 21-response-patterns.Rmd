...
## Change starting location to folder `21-response-patterns`
```{r}

source("rep_functions.R")

change_here(glue("{project_location}/21-response-patterns"))
...
patterns_fit <- mplusModeler(patterns,
                dataout=here("resp_pattrn", "LSAY.dat"),
                modelout=here("resp_pattrn", "patterns.inp") ,
...
out_c4 <- readModels(here("resp_pattrn"), filefilter = "patterns", quiet = TRUE)
...
lca_enum_fit <- mplusModeler(lca_enum, 
    dataout=glue(here("enum_mplus", "lsay.dat")),
    modelout=glue(here("enum_mplus", "c{k}_lca.inp")) ,
    check=TRUE, run = TRUE, hashfilename = FALSE)
})
