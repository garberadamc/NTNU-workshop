...
## Change starting location to folder `05-cfa`
```{r}

source("rep_functions.R")

change_here(glue("{project_location}/05-cfa"))
...
efa_geomin_fit <- mplusModeler(efa_geomin, 
                  dataout=here("efa_mplus", "efa_geomin_oblique.dat"),
                  modelout=here("efa_mplus", "efa_geomin_oblique.inp"),
...
efa_varimax_fit <- mplusModeler(efa_varimax, 
                            dataout=here("efa_mplus", "efa_varimax_orthogonal.dat"),
                            modelout=here("efa_mplus", "efa_varimax_orthogonal.inp"),
                            check=TRUE, run = TRUE, hashfilename = FALSE)
```
...
```{r}
cfa_output <- readModels(here("cfa_mplus"), quiet = TRUE)
```
