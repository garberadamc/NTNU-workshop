...
## Change starting location to folder `16-cond-mediation`
```{r}

source("rep_functions.R")

change_here(glue("{project_location}/16-cond-mediation"))
...
m1_lev2mod_fit <- mplusModeler(m1_lev2mod,
                     ...
                     modelout=here("mplus_files", "m1_lev2mod_vocab.inp"),
...
```{r, eval=FALSE}
mplus.view.plots(here("mplus_files", "m1_lev2mod_vocab.gh5"))
```

```{r, eval=FALSE}
mplus.plot.loop(here("mplus_files", "m1_lev2mod_vocab.gh5"),label =1)
```
...
loop_data <- lapply(1:2, function(k) {
  y_val <- mplus.get.loop.estimates(here("mplus_files", "m1_lev2mod_vocab.gh5"),label=k)
  lower <-   mplus.get.loop.lowerci(here("mplus_files", "m1_lev2mod_vocab.gh5"),label=k)
  upper <-   mplus.get.loop.upperci(here("mplus_files", "m1_lev2mod_vocab.gh5"),label=k)
  x_val <-   mplus.get.loop.xvalues(here("mplus_files", "m1_lev2mod_vocab.gh5")) 
...
m2_contmod_fit <- mplusModeler(m2_contmod,
                  dataout=here("mplus_files", "vocab.dat"),       
                  modelout=here("mplus_files", "m2_contmod_vocab.inp"),
                  check=TRUE, run = TRUE, hashfilename = FALSE)
```
...
loop_data2 <- lapply(1:5, function(k) {
  y_val <- mplus.get.loop.estimates(here("mplus_files", "m2_contmod_vocab.gh5"),label=k)
  lower <-   mplus.get.loop.lowerci(here("mplus_files", "m2_contmod_vocab.gh5"),label=k)
  upper <-   mplus.get.loop.upperci(here("mplus_files", "m2_contmod_vocab.gh5"),label=k)
  x_val <-   mplus.get.loop.xvalues(here("mplus_files", "m2_contmod_vocab.gh5")) 
...
m3_teams_fit <- mplusModeler(m3_teams,
                dataout=here("mplus_files", "teams.dat"),       
                modelout=here("mplus_files", "m3_teams.inp"),
                check=TRUE, run = TRUE, hashfilename = FALSE)
...
```{r, eval=FALSE}
mplus.view.plots(here("mplus_files", "m3_teams.gh5"))
```

Take a look at bootstrap distribution of the indirect effect to view asymptotic shape.
```{r}
mplus.plot.bootstrap.distribution(here("mplus_files", "m3_teams.gh5"), parameter = 38)
```
...
mod_data <- lapply(1:2, function(k) {
  y_val <- mplus.get.moderation.estimates(here("mplus_files", "m3_teams.gh5"),label[k])
  lower <- mplus.get.moderation.lowerci(here("mplus_files"  , "m3_teams.gh5"),label[k])
  upper <- mplus.get.moderation.upperci(here("mplus_files"  , "m3_teams.gh5"),label[k])
  x_val <- mplus.get.moderation.xvalues(here("mplus_files"  , "m3_teams.gh5"))
