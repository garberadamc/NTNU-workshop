...
## Change starting location within the Rproject (`.Rproj`)

1. save stem 
2. `glue()` stem into subsequent labs
```{r}

source("rep_functions.R")

change_here(glue("{project_location}/02-run-models"))
...
m_basic_fit <- mplusModeler(m_basic, 
               dataout=here("basic_mplus", "basic.dat"),
               modelout=here("basic_mplus", "basic.inp"),
...
fem_basic_fit <- mplusModeler(fem_basic, 
                 dataout=here("basic_mplus", "fem_basic.dat"),
                 modelout=here("basic_mplus", "fem_basic.inp"),
                 check=TRUE, run = TRUE, hashfilename = FALSE)
